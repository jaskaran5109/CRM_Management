...
const getStatusBadge = (status) =>
  {
    if (!isLegacyComplaintStatus(status)) {
      return "complaint-detail-status-custom";
    }

    if (status === "pending") {
      return "complaint-detail-status-open";
    }

    if (status === "resolved") {
      return "complaint-detail-status-resolved";
    }

    return "complaint-detail-status-in_progress";
  };
...
export default function ComplaintDetail() {
  ...
  const allowedStatuses = current?.allowedStatuses || [];
  ...
  const handleAssignmentUpdate = async () => {
    ...
    if (assignedUser === currentAssignedUserId) {
      return;
    }
    ...
    <div className="complaint-detail-container">
        ...
        <div className="complaint-detail-header-content">
          <div>
            ...
            <div className="complaint-detail-badges">
              ...
              >
                {formatComplaintStatusLabel(current.status)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="complaint-detail-grid">
        ...
        <div className="complaint-detail-sidebar">
          {canManageWorkflow && (
            <div className="complaint-detail-card">
              ...
              >
                ...
                {isLegacyComplaintStatus(current.status) && (
                  <option value={current.status}>
                    {formatComplaintStatusLabel(current.status)} (legacy)
                  </option>
                )}
          ...
          {isAdmin && (
            <div className="complaint-detail-card">
              <h3 className="complaint-detail-card-title">Assignment</h3>
              <div className="complaint-detail-form-stack">
                ...
                <p className="complaint-detail-helper-text">
                  Assign the complaint without changing its workflow status.
                </p>
                <button
                  onClick={handleAssignmentUpdate}
                  disabled={assignedUser === getEntityId(current.assignedTo)}
                  className="complaint-detail-btn complaint-detail-btn-primary"
                >
                  Save Assignment
                </button>
